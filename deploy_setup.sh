...
echo ">>> Configuring PostgreSQL Database and Schema..."
...
sudo -u postgres psql -d $DB_NAME -c "CREATE SCHEMA IF NOT EXISTS $DB_SCHEMA AUTHORIZATION $DB_USER;"
sudo -u postgres psql -d $DB_NAME -c "GRANT ALL ON SCHEMA $DB_SCHEMA TO $DB_USER;"
# Grant privileges on all existing tables and future tables in the schema
...
echo "  sudo systemctl restart ${PROJECT_NAME}-gunicorn"
echo "=========================================="
